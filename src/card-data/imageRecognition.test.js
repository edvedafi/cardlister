...
describe('Image Recognition', () => {
  ...
  describe('runNLP', () => {
    ...
    it('should pick out the name that is most visible if there is more than one', async () => {
      const input = [
        ...
        },
        {
          ...
          isNumber: false,
          lowerCase:
            "it's a sight that nfl defenses saw time and time again during kupp's incredible 2021 season . nary a game went by in which the pro bowl wideout didn't flash his talent . for spectacular catches , showing total body control to fly high to snag a laser beam from matthew stafford while staying in bounds .",
        },
        {
      ...
      const mockSpinner = jest.fn();
      expect(await runNLP(input, {}, mockSpinner)).toEqual(expected);
    });

    it('should pick out the name that is most visible if there is more than one', async () => {
      const mockSpinner = jest.fn();
      ImageRecognition.callNLP = jest.fn().mockResolvedValue(BigLeague_2023_Willie_Mays_NLP);
      expect(await runNLP(BigLeague_2023_Willie_Mays, {}, mockSpinner)).toEqual({
        player: 'Willie Mays',
      });
    });
    ...
    it('should pick out an initialed name from the text', async () => {
      const input = [
        ...
        },
        {
          ...
          isNumber: false,
          lowerCase:
            "big and strong , brown can be a nightmare in single coverage . when he goes airborne , he's virtually unstoppable . using his size and strength to box out a cornerback , the pro bowl wideout finishes the play off by using his glue - like hands to snag the ball and taps his toes in bounds for the catch .",
        },
        {
          word: 'No. TL - AB',
          words: ['No.', 'TL', '-', 'AB'],
      ...
      ask.mockResolvedValue('enter');
      const mockSpinner = jest.fn();
      expect(await runNLP(input, {}, mockSpinner)).toEqual({
        player: 'A.J. Brown',
      });
    });
    ...
    it('should pick not return symbols', async () => {
      const input = [
        ...
        },
        {
          ...
          isNumber: false,
          lowerCase:
            "you've heard of bird watching ? in dallas , fans prefer to spend their time lamb watching . grabbing their binoculars , they keep them focused on the skies above the field , where they're likely to spot their favorite high - flying receiver taking to the air to haul in a pass from dak prescott .",
        },
        {
          word: 'TOE THE',
          words: ['TOE', 'THE'],
    ...
    it("should find a name with an ' in it", async () => {
      const input = [
        ...
        },
        {
          ...
          isNumber: false,
          lowerCase:
            "y.a. tittle was the first giants player to record at least 400 passing yards in a game , throwing for 505 in 1962. three players have joined him on that list since : phil simms , eli manning and jones . new york's current qb accomplished it for the first time with a 402 - yard day vs. new orleans in week 4 of 2021 .",
        },
        {
    ...
    it('should use both first and last name to do the count', async () => {
      ...
      const mockSpinner = jest.fn();
      expect(await runNLP(Chronicles_2021_Crusade_McKenzie, {}, mockSpinner)).toEqual({
  ...
  describe('Data Extraction: ', () => {
    describe('card number', () => {
      it('should find a card number that is prefixed with "no. "', async () => {
        ...
        const mockSpinner = jest.fn();
        expect(await extractData(input, {}, { isSet: false }, mockSpinner)).toMatchObject({
          cardNumber: '307',
        });
      });

      it('should not include the . if the text is read with a space ("NO . 6" should be 6)', async () => {
        const mockSpinner = jest.fn();
        expect(await extractData(Chronicles_2021_Pujols, {}, { isSet: false }, mockSpinner)).toMatchObject({
          cardNumber: '6',
        });
      });
    });
    ...
    describe('Year', () => {
      it('should be based on copyright if it is available', async () => {
        const mockSpinner = jest.fn();
        expect(await extractData(Score_2022_Adian_Hutchinson, {}, { isSet: false }, mockSpinner)).toMatchObject({
          year: '2022',
        });
      });
      it('should do year + 1 if the value comes out of the body of the card', async () => {
        const mockSpinner = jest.fn();
        expect(await extractData(Vintage_Topps, {}, { isSet: false }, mockSpinner)).toMatchObject({
          year: '1963',
        });
      });
    });
    ...
    describe('Vintage Card makers', () => {
      it('should include Philadelphia as P. C. G.', async () => {
        const mockSpinner = jest.fn();
        expect(await extractData(Vintage_Philadelphia, {}, { isSet: false }, mockSpinner)).toMatchObject({
